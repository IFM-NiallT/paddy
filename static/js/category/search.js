...
export const categorySearch = (function() {
  'use strict';
  ...
  function updateModalResults(data) {
      ...
      data.Data.forEach((product) => {
          ...
          row.innerHTML = `
              ...
              <td>${utils.escapeHtml(product.Description)}</td>
              <td>${utils.escapeHtml(product.Category?.Description || "N/A")}</td>
              <td>
  ...
  function init() {
      ...
      if (categorySearchInput) {
          const debouncedSearch = utils.debounce(searchCategories, 300);
      ...
      if (modalSearchInput) {
          const debouncedSearch = utils.debounce(searchProductsModal, 300);
